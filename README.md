...
成本由「字幕长度 → 分块策略 → 模型单价」决定。字幕按 `字数 ÷ 2` 估算 token，预算为 `contextBudget`（默认 8000）× 60% = 4800 tokens/块，块间重叠 30 秒：

| 视频 | 字幕 tokens | LLM 调用 | 输入 tokens | 输出 tokens |
| --- | --- | --- | --- | --- |
| 5 分钟 | ~750 | 2 次（1 map + 1 reduce） | ~2,000 | ~1,000 |
| 30 分钟 | ~4,000 | 2 次 | ~5,500 | ~2,000 |
| 2 小时 | ~18,000 | 5 次（4 map + 1 reduce） | ~27,000 | ~4,000 |

结果卡片上的「输入约 N tokens」是字幕部分实测估算；**重复打开同视频 + 分 P + 同模型命中缓存，成本为 0**（仅「重新生成」消耗额度）。

### 2026 国内 Coding Plan 价格与推荐

> 价格随官方调整变动，以各官网为准：DeepSeek、MiniMax、Kimi、智谱。

| 方案 | 价格（2026 年中） | 一次 30 分钟分析约 | 推荐用法 |
| --- | --- | --- | --- |
| **DeepSeek**（V4-Flash 按量） | 输入 ~¥1/百万（缓存命中更低），输出 ~¥2/百万 | **¥0.01-0.02** | 省钱主力：课程总结/大纲的首选；注意 2026-07-24 旧 `deepseek-chat` 名停用，用新模型名 |
| **MiniMax M3**（Token Plan ¥29/月起，或按量 ¥2.1/百万输入 ≤512K） | 按量输入 ¥2.1/百万 | **¥0.02-0.04** | 已有 Token Plan 就物尽其用：分析 + 答疑主力，1M 上下文适合长课 |
| **Kimi**（K2.5/K2.6 按量，行板会员 ¥49/月含等额 API 券） | 输入 ~¥3/百万级 | **¥0.05-0.08** | 答疑讲解 + 联网搜索（BiliNote 联网用其 `$web_search` 内置工具） |
| **智谱 GLM**（Coding Plan 订阅；GLM-4 每月 100 万免费 tokens） | 订阅制 | 免费额度内 ¥0 | 已有 Coding Plan 可当分析主力；免费额度适合尝鲜 |
| **阿里百炼订阅**（Qwen/GLM/Kimi K2.5 打包） | ~¥7.9/月 | 订阅内覆盖 | 低频用户最省心的包月 |
| **语音转写（无字幕时）** | Groq / 硅基流动有免费额度；OpenAI whisper $0.006/分钟 | 30 分钟 ~¥0-1.3 | 转写费可能高于分析费，优先用免费额度（见上文「转写服务怎么选」） |

**分工建议**：总结/大纲（量最大、价格敏感）走 DeepSeek 或 MiniMax M3；答疑/讲解（质量敏感）走 Kimi K2.6 或 MiniMax M3；联网搜索走 Kimi。任务级模型路由（按任务自动分配）在路线图上；当前切换激活 Profile 即可。
